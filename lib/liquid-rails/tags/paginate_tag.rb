...
module Liquid
  module Rails
    class PaginateTag < ::Liquid::Block
      Syntax = /(#{::Liquid::QuotedFragment})\s*(by\s*(\d+))?/.freeze
        ...
        context.stack do
          ...
          if collection.nil?
            raise ::Liquid::ArgumentError,
                  "Cannot paginate collection '#{@collection_name}'. Not found."
          end
          ...
          pagination['previous']        = link('&laquo; Previous'.html_safe, current_page - 1) unless current_page <= 1
          unless page_count < current_page + 1
            pagination['next'] =
              link('Next &raquo;'.html_safe, current_page + 1)
          end
          pagination['parts'] = []
